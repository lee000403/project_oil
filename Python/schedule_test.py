...
def selenium_funtion():
    try:
        ...
        engine = create_engine(
            "mysql://{user}:{pw}@localhost/{db}".format(user='root', pw='!yojulab*', db='db_oil'))
        ...
        for j in range(1, 4):
            ...
            if j == 1:
                for i in range(1, len(area_oil)+1):
                    oil_price_list = []
                    area_oil_re = driver.find_element(
                        By.XPATH, "//*[@id='numbox']/tr[{}]/td[3]".format(i))
            ...
            # 고급 휘발유
                ...
                for i in range(1, len(area_oil)+1):
                    oil_price_list = []
                    area_oil_re = driver.find_element(
                        By.XPATH, "//*[@id='numbox']/tr[{}]/td[2]".format(i))
                    oil_price_value = area_oil_re.get_attribute("innerText")
                    oil_price_list.append("a-{}".format(i))
                    oil_price_list.append(oil_price_value)
                    oil_price_list.append("o-0{}".format(j))
            ...
            # 경유
                ...
                for i in range(1, len(area_oil)+1):
                    oil_price_list = []
                    area_oil_re = driver.find_element(
                        By.XPATH, "//*[@id='numbox']/tr[{}]/td[4]".format(i))
                    oil_price_value = area_oil_re.get_attribute("innerText")
                    oil_price_list.append("a-{}".format(i))
                    oil_price_list.append(oil_price_value)
                    oil_price_list.append("o-0{}".format(j))
                    ...
                    oil_price.append(oil_price_list)
        driver.close()
        df = pd.DataFrame(oil_price, columns=[
                        "area_unique", "oil_price", "oil_unique", "date_oil"])
...
schedule.every(1).days.at("12:00").do(selenium_funtion)
